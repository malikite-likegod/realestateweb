...
export function CommunityForm({ initial }: Props) {
  ...
  // Fetch the distinct MLS areas for the City (Area) dropdown
  useEffect(() => {
  ...
  // Options sorted by their consumer-friendly label. If editing a community whose
  // stored city is no longer in the feed, keep it selectable by prepending it.
  const areaOptions = (() => {
    const values = form.city && !areas.includes(form.city) ? [form.city, ...areas] : areas
    return values
      .map(city => ({ value: city, label: getDisplayCity(city) }))
        ...
        <Card>
          ...
          <div className="flex flex-col gap-4">
            ...
            <Select
              ...
              options={areaOptions}
              hint="Areas come from the MLS feed. For-sale residential listings for this area are shown on the community page."
              onChange={(e) => setForm(f => ({ ...f, city: (e.target as HTMLSelectElement).value }))}
            />
            <Textarea
              label="Description"
